...
def showMove(pics):
    ...
    while running:
        for evnt in event.get():               
            if evnt.type == KEYDOWN and evnt.key == K_ESCAPE:
                return []
...
while running:
    for evnt in event.get():               
        ...
        if evnt.type == MOUSEBUTTONUP and evnt.button==1:
            ...
            area = [min(stx,ex),min(sty+offy,ey+offy),max(ex,stx),max(sty+offy,ey+offy)]
            w = area[2]-area[0]
            h = area[3]-area[1]
            if w>h:
                getMove(area)
